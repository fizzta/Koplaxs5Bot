...
cl = LINETCR.LINE()
cl.login(token="EoSpqMMHBAGuxZdTo27a.O1G12Pn1ZFYhtQQBj5Mx2G.UetXPWFYRVmAelAF2afs2D6Cl1njyBh0bGsvvrSe62o=")
cl.loginResult()

ki = LINETCR.LINE()
ki.login(token="EoOCbSxsEydsMvmvr0B2.V+19+9fB7bQx4L5S1XzISG.lNLBp31GFQWaRhpTRgs6wiRgNk6FZaIXlxkVM+3NmWQ=")
ki.loginResult()

ki2 = LINETCR.LINE()
ki2.login(token="EoHzSEjOeBB6RswHBX48.qcvDiPb+ZOpG+rVzkcuT/a.DwM/QxAokJf9mr0OezBnXOYP4UVtfiUk43TC+r+EwkE=to")
ki2.loginResult()
...
List Harga Bisa Langsung Pc ke creator
...
def bot(op):
        ...
        if op.type == 13:
            if mid in op.param3:
              if wait["autoJoin"] == True:
                if op.param2 in Bots + owner + admin + staff:
    ...
    #------------------------------------------------------------
              ...
              if msg.from_ in owner + admin + staff:
                if msg.toType == 2:
                    X = cl.getGroup(msg.to)
...
#-----------------------------------------------
    ...
    #-------------Fungsi Leave Group Start---------------#
            elif msg.text in ["Hushus","Cabut","Bye"]:
              if msg.from_ in owner or admin:
                if msg.toType == 2:
                  cl.getGroup(msg.to)
        ...
        #-----------------------------------------
              ...
              if msg.from_ in owner:
                if msg.toType == 2:
                    ...
                    cl.sendText(msg.to,"Psycho Team Mau Bersih² Group Sampah Nih")
...
#-----------------------------------------------
       ...
       #-------------Fungsi Respon Start---------------------#
              ...
              if msg.from_ in owner + admin +staff :
                cl.sendText(msg.to,"꧁рsчÇнø тздм вøт꧂")
                ki.sendText(msg.to,"꧁рsчÇнø тздм вøт꧂")
                ki2.sendText(msg.to,"꧁рsчÇнø тздм вøт꧂")
                ki3.sendText(msg.to,"꧁рsчÇнø тздм вøт꧂")
                ki4.sendText(msg.to,"꧁рsчÇнø тздм вøт꧂")
      ...
      #-------------Fungsi Balesan Respon Finish---------------------#
       ...
       #-------------Fungsi Speedbot Start---------------------#
              ...
              if msg.from_ in owner and admin and staff:
                start = time.time()
